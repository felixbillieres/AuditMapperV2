...
const KillchainVisualization: React.FC<KillchainVisualizationProps> = ({
  ...
  const phases = useMemo((): Phase[] => {
    ...
    const phaseWidth = 350; // Taille standard comme v1
    const margin = 60;
    const phaseSpacing = 60; // Plus d'espace entre les phases
  ...
  const graphElements = useMemo(() => {
    ...
    phases.forEach((phase, phaseIndex) => {
      ...
      const phaseBottomY = 580; // Fin des nœuds
      const phaseCenterY = (phaseTopY + phaseBottomY) / 2; // Centre vertical
      
      // Calculer la disposition optimale
      const nodeSize = 45; // Taille des nœuds
      const nodeSpacing = 70; // Espacement entre les nœuds
    ...
    const cy = cytoscape({
      ...
      style: [
        // Style des nœuds de phase (zones de killchain)
        {
          ...
          style: {
            'background-color': (ele) => ele.data('phaseColor') + '15',
  ...
  return (
    <div className="relative w-full h-full bg-slate-900">
      ...
      <LegendButton
        title="Légende BloodHound Killchain"
        items={[
